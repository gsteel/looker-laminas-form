...
SHELL=/bin/bash
PHP_VERSION := 8.4
IMAGE_NAME := gsteel/looker-laminas-form
IMAGE_ID := $(shell docker images -q ${IMAGE_NAME} | xargs)
WORK = /app
UID := $(shell id -u)
GID := $(shell id -g)
DOCKER_PHP=-it --user ${UID}:${GID} -w ${WORK} -v ${PWD}:/app --rm ${IMAGE_NAME}
...
docs-lint: .markdownlint.json ## Lint documentation
	...
	@docker run -it -w /app -v ${PWD}:/app --rm ${MDLINT_IMAGE} ./*.md
.PHONY: docs-lint

check-links: ## Check documentation links
	@$(call MK_INFO,"Checking links in documentation files")
	@docker run -it -w /app -v ${PWD}:/app --rm ${LINK_CHECKER_IMAGE} ./*.md
.PHONY: check-links
...
mutants: docker ## Run mutation tests
	...
	@docker run $(DOCKER_PHP) tools/infection/vendor/bin/infection --configuration=mutants.json
.PHONY: mutants
...
infection-ci:
	...
	tools/infection/vendor/bin/infection --configuration=mutants.json
.PHONY: infection-ci
